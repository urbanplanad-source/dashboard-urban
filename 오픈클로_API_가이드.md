# 어반플랜애드 대시보드 API 가이드 (오픈클로용)
> 최종 업데이트: 2026-05-11 / Apps Script 버전 12 (v13 리뷰모니터링 포함) / 대시보드 정규화 규칙 v14 반영
...
```
> 필수 업무 카운트 초기화 + 완료 처리된 서브업무 제거 + 새 달 시트 구성
...
오픈클로가 비용을 등록·수정·삭제하면 대시보드에 즉각 반영된다.
...
오픈클로가 작성한 글을 대시보드 **글 보관함** 탭에 저장한다.  
...
> 대시보드에서 직접 삭제(발행 완료 후)도 가능하므로 오픈클로는 저장만 담당해도 됨.

---

### GET — 초안 목록 조회

별도 엔드포인트 없음. `summary` GET에 `drafts` 배열이 포함된다.

```javascript
const res = await fetch(API + '?action=summary');
const { data } = await res.json();
const drafts = data.drafts; // 전체 초안 목록
```
...
- 리뷰 수가 N개 증가했다면 키미클로는 신규 방문자 리뷰 N개를 **모두** 확인해야 한다.  
...
오픈클로가 캘린더에 거래처 업무를 등록하려면 `addSubJob`을 사용하고 `dueDate`를 반드시 포함한다.
...
오픈클로가 캘린더에 내 사업 공통업무를 등록하려면 `commonTaskAdd`를 사용한다.
