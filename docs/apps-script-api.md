...
>
> 대시보드 버튼은 `monthlyReset` 단일 요청으로 `note`가 `"완료"` 또는 `"완료 (...)"`로 시작하는 완료된 서브업무 삭제, 필수 업무 카운트 초기화, 새 달 시트 구성을 서버에서 함께 처리한다. 완료 체크하지 않은 서브업무는 다음 달로 이월한다. `month`는 새 운영월(`YYYY-MM`)을 전달한다.
>
> 마감월은 `prevMonth()`로 암묵 고정하지 않는다. 미리보기에서 사용자가 마감월을 선택하고, 새 운영월은 마감월의 다음 달로 계산한다. 미래 월은 차단하고, 진행 중인 달이거나 이미 백업 이력이 있는 달은 경고한다. 월말에 눌러 이번 달 자료를 통째로 건너뛰거나 같은 달을 두 번 마감하는 실수를 막기 위한 장치다.
>
> 대시보드 월별 상담 백업(`consultHistories`)은 **브라우저 로컬 보조 이력**이다. Sheets `Consults` 원본을 대체하는 영구 백업이 아니며, 다른 브라우저나 저장소 초기화 후에는 표시되지 않는다. 화면에도 같은 문구를 표시한다.
>
> 대시보드는 아래 단계를 구분해 처리한다. 서버 반영 여부와 화면 갱신 여부를 절대 섞지 않는다.
>
> | 단계 | 의미 | 로컬 반영 | 재실행 안내 |
> |------|------|-----------|-------------|
> | `preflight_loading` | 전월 상담 원격 조회 | 없음 | — |
> | `preflight_failed` | 상담 조회 실패 | 없음. Sheets도 미변경 | 재시도 안내 |
> | `reset_posting` | `monthlyReset` POST 1회 | 없음 | — |
> | `reset_failed` | POST 실패 | 없음. Sheets도 미변경 | 재시도 안내 |
> | `reset_applied_refreshing` | `summary` 재조회 | — | — |
> | `complete` | 정상 완료 | 서버 `monthlyJobs`를 그대로 반영 | — |
> | `reset_applied_refresh_failed` | **서버는 반영됨**, 화면 갱신만 실패 | 상담 백업 이력만 보존 | **재실행 금지.** 동기화 안내 |
>
> `monthlyReset` POST가 성공한 뒤 `summary` 조회가 실패해도 초기화 실패가 아니다. 이 상태에서 재실행을 안내하면 중복 처리 위험이 생기므로 "Sheets 초기화는 완료되었으니 동기화하라"고만 안내한다.
>
> 초기화 후 `summary`가 비었거나 `monthlyJobs`가 없으면 **자동으로 `addSubJob`/`updateSubJob` 보강을 하지 않는다.** 과거에는 자동 보강 때문에 정상 이월된 서브업무가 새 `jobId`로 중복 생성될 수 있었다. 지금은 화면 갱신 실패로 처리하고 사용자가 동기화 후 직접 확인한다.
>
> 초기화 버튼은 `useRef` 기반 in-flight 가드로 중복 실행을 막고, 진행 중에는 비활성화한다.
>
...
>
> **라이브 배포 버전은 저장소에서 확인할 수 없다.** 2026-09 기준 라이브 Web App은 버전 21(2026-07-19)이고, `monthlyReset`을 추가한 v20 배포는 보관 처리되어 있어 v16 패치와 동일하다고 단정할 수 없다. 따라서 대시보드는 서버가 무엇을 삭제·초기화할지 **예측하지 않는다.**
>
> - 미리보기는 로컬이 아는 사실(kind, 완료 여부, targetCount)로 "영향 대상"만 분류해 보여주고, 서버 동작을 단정하는 문구를 쓰지 않는다.
> - 초기화 후에는 `summarizeResetOutcome`이 초기화 전후 `monthlyJobs`를 비교해 실제로 삭제된 업무, 카운트가 0이 된 업무, 메모가 비워진 업무를 집계해 사용자에게 보고한다.
> - `monthlyReset` 응답에 `deletedCompletedSubJobs` 같은 처리 건수가 있으면 함께 표시하고, 없으면 생략한다. 응답 필드 유무에 의존하지 않는다.
>
...
| 고정비 다음 달 수동 적용 | `addExpense` | 기존 고정비 행 기준으로 date를 다음 달 같은 일자로 변경, 중복 조합은 추가하지 않음 |
...
>
> **상담 화면 진입만으로는 어떤 POST도 발생하지 않는다.** 과거에는 원격 조회 결과가 비었을 때 로컬 상담 전체를, 원격에 없는 항목은 개별로 `addConsult` POST 했다. 수백 건이 동시 전송되어 `Failed to fetch`와 Apps Script 동시 실행 경합을 일으켰고, fire-and-forget이라 사용자에게는 실패가 보이지 않았다. 현재는 자동 전송을 하지 않는다.
>
> 원격에 없는 로컬 상담은 삭제하지 않고 계속 화면에 표시하며, 건수만 안내 문구로 알린다. Sheets 보강이 필요하면 `scripts/consult-backfill.mjs`로 처리한다. 이 스크립트는 상담 데이터를 `--input` 파일에서 읽고, endpoint를 하드코딩하지 않으며, `--apply` 없이는 POST하지 않고, 순차 전송한다.
